...
kangaroo::kangaroo( ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv ) :
	...
	hz(50),
	true_max(0), true_min(0), target_pos(0)

{
	...
	std::string portname;
	nh_priv.getParam("port", portname);
	ROS_INFO(portname.c_str());
...
bool kangaroo::start( )
{
	...
	js_sub = nh.subscribe("cmd_vel",1,&kangaroo::JoystickCB,this);
...
bool kangaroo::set_channel_pos(double position, double speedLimit, unsigned char address, char channel)
{
	if (!is_open() && !open())
		return false;

	unsigned char buffer[18];
	int num_of_bytes = write_kangaroo_position_command(address, channel, position, (int)speedLimit, buffer);
	if (0 > write(fd, buffer, num_of_bytes))
	{
		ROS_ERROR("Failed to update channel %c: %s", channel, strerror(errno));
		close();
...
void kangaroo::JointStateCB( const ros::WallTimerEvent &e )
{
	try
	{
		...
		position_ln = get_parameter((unsigned char)128, '1', (unsigned char)1) /4;	// position for ch1
		velocity_ln = get_parameter((unsigned char)128, '1', (unsigned char)2) / 4;	// velocity for ch1
...
void kangaroo::setPosition( double target_pos_, int speed)
{
		boost::mutex::scoped_lock output_lock(output_mutex);
		set_channel_pos( target_pos_,speed,128, '1');
}
...
void kangaroo::reportState()
{

	double turns = pos_rad/ (2*PI) ;
	ROS_INFO("max / min / range : %d, %d, %d, %d lines ", set_max, set_min,range_ln,range_ln);
	ROS_INFO("Current Pos/Lines: %d, Position/Rads : %.3f, ratio:%.3f, target:%d",position_ln , turns, pos_ratio, target_pos);
...
{
//	if( target_pos == position_ln ) setSpeed(0);
	if(receivedInput)
	{
		setPosition(target_pos,7000);
		receivedInput = false;
